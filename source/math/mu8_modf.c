...
#			elif (__has_builtin(__builtin_modfl))
				mu0_fp128_t r;
				mu0_fpex_t  q;
				r  = mu0_fp128(__builtin_modfl(mu0_const_fpex(x), &q));
				*f = mu0_const_fp128(q);
				return r;
#			else
				mu0_fp128_t r;
				mu0_fpex_t  q;
				r  = mu0_fp128(modfl(mu0_const_fpex(x), &q));
				*f = mu0_const_fp128(q);
				return r;
#			endif
#		else
...
#			elif (__has_builtin(__builtin_modff))
				mu0_fp16_t r;
				mu0_fp32_t q;
				r  = mu0_fp16(__builtin_modff(mu0_const_fp32(x), &q));
				*f = mu0_fp16(q);
				return r;
#			else
				mu0_fp16_t r;
				mu0_fp32_t q;
				r  = mu0_fp16(modff(mu0_const_fp32(x), &q));
				*f = mu0_fp16(q);
				return r;
#			endif
#		else
