
//
// # -*- coding: utf-8, tab-width: 3 -*-

//                                                           ___                                            //
//                                                          /   \                                           //
//                                            _   _  _   _  \ O /                                           //
//                                           | | | || | | | / _ \                                           //
//                                           | |_| || |_| |( (_) )                                          //
//                                           | ._,_| \___/  \___/                                           //
//                                           | |                                                            //
//                                           |_|                                                            //

// mu8_zsqrt.c
//
// Copyright (C) 2023 mu578. All rights reserved.
//

#include <mu8/mu8_complex_annex.h>
#include <mu8/mu8_math_annex.h>

void mu8_zsqrt_fp128 (mu0_fp128_t * zr, mu0_fp128_t * zi, const mu0_fp128_t ar, const mu0_fp128_t ai)
{
#	if MU0_HAVE_FASTMATH
	mu8_zpolar_fp128(zr, zi
		, mu8_sqrt_fp128(mu8_zabs_fp128(ar, ai))
		, mu8_zarg_fp128(ar, ai) * __mu0_fp128_const__(0.5)
	);
#	else
	if (mu8_isinf_fp128(ai)) {
		*zr = mu0_fp128_inf;
		*zi = ai;
	} else if (mu8_isinf_fp128(ar)) {
		if (ar > mu0_fp128_zero) {
			*zr = ar;
			*zi = mu8_isnan_fp128(ai) ? ai : mu8_copysign_fp128(mu0_fp128_zero, ai);
		} else {
			*zr = mu8_isnan_fp128(ai) ? ai : mu0_fp128_zero;
			*zi = mu8_copysign_fp128(ar, ai);
		}
	} else {
		mu8_zpolar_fp128(zr, zi
			, mu8_sqrt_fp128(mu8_zabs_fp128(ar, ai))
			, mu8_zarg_fp128(ar, ai) * __mu0_fp128_const__(0.5)
		);
	}
#	endif
}

void mu8_zsqrt_fp64  (mu0_fp64_t  * zr, mu0_fp64_t  * zi, const mu0_fp64_t  ar, const mu0_fp64_t  ai)
{
#	if MU0_HAVE_FASTMATH
	mu8_zpolar_fp64(zr, zi
		, mu8_sqrt_fp64(mu8_zabs_fp64(ar, ai))
		, mu8_zarg_fp64(ar, ai) * __mu0_fp64_const__(0.5)
	);
#	else
	if (mu8_isinf_fp64(ai)) {
		*zr = mu0_fp64_inf;
		*zi = ai;
	} else if (mu8_isinf_fp64(ar)) {
		if (ar > mu0_fp64_zero) {
			*zr = ar;
			*zi = mu8_isnan_fp64(ai) ? ai : mu8_copysign_fp64(mu0_fp64_zero, ai);
		} else {
			*zr = mu8_isnan_fp64(ai) ? ai : mu0_fp64_zero;
			*zi = mu8_copysign_fp64(ar, ai);
		}
	} else {
		mu8_zpolar_fp64(zr, zi
			, mu8_sqrt_fp64(mu8_zabs_fp64(ar, ai))
			, mu8_zarg_fp64(ar, ai) * __mu0_fp64_const__(0.5)
		);
	}
#	endif
}

void mu8_zsqrt_fp32  (mu0_fp32_t  * zr, mu0_fp32_t  * zi, const mu0_fp32_t  ar, const mu0_fp32_t  ai)
{
#	if MU0_HAVE_FASTMATH
	mu8_zpolar_fp32(zr, zi
		, mu8_sqrt_fp32(mu8_zabs_fp32(ar, ai))
		, mu8_zarg_fp32(ar, ai) * __mu0_fp32_const__(0.5)
	);
#	else
	if (mu8_isinf_fp32(ai)) {
		*zr = mu0_fp32_inf;
		*zi = ai;
	} else if (mu8_isinf_fp32(ar)) {
		if (ar > mu0_fp32_zero) {
			*zr = ar;
			*zi = mu8_isnan_fp32(ai) ? ai : mu8_copysign_fp32(mu0_fp32_zero, ai);
		} else {
			*zr = mu8_isnan_fp32(ai) ? ai : mu0_fp32_zero;
			*zi = mu8_copysign_fp32(ar, ai);
		}
	} else {
		mu8_zpolar_fp32(zr, zi
			, mu8_sqrt_fp32(mu8_zabs_fp32(ar, ai))
			, mu8_zarg_fp32(ar, ai) * __mu0_fp32_const__(0.5)
		);
	}
#	endif
}

void mu8_zsqrt_fp16  (mu0_fp16_t  * zr, mu0_fp16_t  * zi, const mu0_fp16_t  ar, const mu0_fp16_t  ai)
{
#	if MU0_HAVE_FASTMATH
	mu8_zpolar_fp16(zr, zi
		, mu8_sqrt_fp16(mu8_zabs_fp16(ar, ai))
		, mu8_zarg_fp16(ar, ai) * __mu0_fp16_const__(0.5)
	);
#	else
	if (mu8_isinf_fp16(ai)) {
		*zr = mu0_fp16_inf;
		*zi = ai;
	} else if (mu8_isinf_fp16(ar)) {
		if (ar > mu0_fp16_zero) {
			*zr = ar;
			*zi = mu8_isnan_fp16(ai) ? ai : mu8_copysign_fp16(mu0_fp16_zero, ai);
		} else {
			*zr = mu8_isnan_fp16(ai) ? ai : mu0_fp16_zero;
			*zi = mu8_copysign_fp16(ar, ai);
		}
	} else {
		mu8_zpolar_fp16(zr, zi
			, mu8_sqrt_fp16(mu8_zabs_fp16(ar, ai))
			, mu8_zarg_fp16(ar, ai) * __mu0_fp16_const__(0.5)
		);
	}
#	endif
}

/* EOF */