...
void mu8_zexp2_fp128 (mu0_fp128_t * zr, mu0_fp128_t * zi, const mu0_fp128_t ar, const mu0_fp128_t ai)
{
	if (ai == mu0_fp128_zero) {
		*zr = mu8_exp2_fp128(ar);
		*zi = mu0_fp128_zero;
	} else {
		mu8_zpow_fp128(zr, zi, mu0_fp128_two, mu0_fp128_zero, ar, ai);
	}
}
...
void mu8_zexp2_fp64  (mu0_fp64_t  * zr, mu0_fp64_t  * zi, const mu0_fp64_t  ar, const mu0_fp64_t  ai)
{
	if (ai == mu0_fp64_zero) {
		...
		*zi = mu0_fp64_zero;
		...
		mu8_zpow_fp64 (zr, zi, mu0_fp64_two, mu0_fp64_zero, ar, ai);
	}
}
...
void mu8_zexp2_fp32  (mu0_fp32_t  * zr, mu0_fp32_t  * zi, const mu0_fp32_t  ar, const mu0_fp32_t  ai)
{
	if (ai == mu0_fp32_zero) {
		...
		*zi = mu0_fp32_zero;
		...
		mu8_zpow_fp32 (zr, zi, mu0_fp32_two, mu0_fp32_zero, ar, ai);
	}
}
...
void mu8_zexp2_fp16  (mu0_fp16_t  * zr, mu0_fp16_t  * zi, const mu0_fp16_t  ar, const mu0_fp16_t  ai)
{
	if (ai == mu0_fp16_zero) {
		...
		*zi = mu0_fp16_zero;
		...
		mu8_zpow_fp16 (zr, zi, mu0_fp16_two, mu0_fp16_zero, ar, ai);
	}
}
